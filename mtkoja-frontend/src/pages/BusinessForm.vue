    ...
    try {
      ...
      try {
        ...
        if (isEditMode.value) {
          ...
          response = await axios.put(`http://localhost:8000/api/businesses/${businessId.value}`, formDataToSubmit, {
            headers: {
              'Authorization': `Bearer ${token}`,
          ...
          // Create new business
          response = await axios.post('http://localhost:8000/api/businesses', formDataToSubmit, {
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json'
            }
        ...
        if (uploadedFiles.value.length > 0 && response.data.business) {
          ...
          try {
            const imageResponse = await axios.post(`http://localhost:8000/api/businesses/${businessId}/images`, formData, {
              headers: {
                'Authorization': `Bearer ${token}`,
