...
$('body').on('change', '.js-example-basic-single4', function () {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      // Handle both array format (from admin-listing.js) and object format (from admin_edit_form)
    ...
    error: function (xhr, status, error) {
      ...
      var content = {};
      content.message = 'دریافت شهرها ناموفق بود. لطفاً دوباره تلاش کنید.';
      content.title = 'خطا';
...
$('#listingForm').on('submit', function (e) {
  ...
  if (can_listing_add == 0) {
    bootnotify('لطفاً یک برنامه بخرید تا بتوانید یک لیست اضافه کنید!', 'هشدار', 'warning');
    return false;
  } else if (can_listing_add == 2) {

    $("#checkLimitModal").modal('show');

    bootnotify("حداکثر تعداد لیست‌ها به پایان رسیده است.", 'هشدار', 'warning');
    return false;
  }
  $('.request-loader').addClass('show');
