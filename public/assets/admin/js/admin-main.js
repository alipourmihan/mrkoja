$(function ($) {
  ...
  $(document).ready(function () {
    $(".updateButton").on('click', function () {
      ...
      var message = "حداکثر تعداد لیست‌ها به پایان رسیده است.";
      bootnotify(message, 'هشدار', 'warning')
    });
  });
  $(document).ready(function () {
    $(".noPackage").on('click', function () {

      var message = "لطفاً یک برنامه بخرید تا بتوانید یک تیکت اضافه کنید.";
      bootnotify(message, 'هشدار', 'warning')
    });
  });
      ...
      beforeSend: function (jqXHR, settings, event) {
        if (settings.type == 'POST' && settings.url.indexOf('user/qr-code/generate') == -1) {
          ...
          if ($("button[disabled='disabled']").length > 0) {
            ...
            bootnotify('این نسخه نمایشی است. شما نمی‌توانید چیزی را اینجا تغییر دهید!', 'نسخه نمایشی', 'هشدار')
          jqXHR.abort(event);
        }
      },
      complete: function () {
...
******************************************************/
  ...
  ******************************************************/
    ...
    $.ajaxSetup({
      beforeSend: function (jqXHR, settings) {
        if (settings.type == 'POST' && status == 0) {
          ...
          var message = "حساب شما غیرفعال یا در انتظار تأیید است. لطفاً با مدیریت تماس بگیرید!";
          bootnotify(message, 'هشدار', 'warning')
          jqXHR.abort(event);
        }
      },
  ...
  if(datepickers.length > 0){
    $.each(datepickers, function () {
      var that = $(this);
      that.attr('dateId','date_id_'+counter)
      let elId=that.attr('dateId');
      var date = that.parents('.date').find('input[type="text"]').val();
      let el1=$("#"+elId);
     if($('#date_id_1').length>0){
       $('#date_id_1').remove();
     }
     
         
        let attr=that.parents('.date').find('input[type="text"]').attr('id');
        
        that.after('<input id="'+elId+'" type="hidden" name="'+attr+'" d-role="dateKeeper" value="" >')
        that.parents('.date').find('input[type="text"]').removeAttr('name');
        el1=$("#"+elId);
        var opt = {
         dateFormat: 'yyyy-MM-dd',
         textFormat: 'yyyy-MM-dd',
         placement: 'bottom',
         targetTextSelector:'#'+that.parents('.date').find('input[type="text"]').attr('id'),
         targetDateSelector:'#'+elId,
         trigger: 'click',
         isGregorian: false,
         englishNumber: false,
        ...
        that.MdPersianDateTimePicker(opt);
  ...
  ******************************************************/
      ...
      success: function (data) {
        ...
        if (data == "downgrade") {
          ...
          var content = {};
            ...
            content.message = "حد مجاز بسته شما به پایان رسیده یا از آن فراتر رفته است!"
  ...
  // Store edit data for modal
  var editModalData = null;

  // Fix aria-hidden issue and populate form fields when modal is shown
  $('#editModal').on('shown.bs.modal', function () {
    $(this).attr('aria-hidden', 'false');
    
    if (editModalData) {
      var datas = editModalData;
  ...
  $('#editModal').on('hidden.bs.modal', function () {
    ...
    editModalData = null; // Clear data when modal is closed
  });
  ...
  $(".editBtn").on('click', function () {
    ...
    // Store data for modal shown event
    editModalData = datas;
...
******************************************************/
  ...
  ******************************************************/
  ...
  // Remove legacy language direction change handlers (single-language mode)
