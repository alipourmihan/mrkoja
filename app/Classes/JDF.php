...
function jdate_en($format, $timestamp = '', $none = '', $time_zone = 'Asia/Tehran') {
    return $this->jdate($format,$timestamp, $none, $time_zone, 'en');
}
/*	F	*/
...
function jalaliDate($time){
    $date=$time;
    if (is_rtl() && get_locale_key($GLOBALS['language'])=='fa' && isset($time) && $time!=null && $time!=''){
        $time=explode(' ',$time);
        $part=explode('-',$time[0]);
        $part=$this->gregorian_to_jalali($part[0],$part[1],$part[2]);
        $part[1]=strlen($part[1])<=1?'0'.$part[1]:$part[1];
        $part[2]=strlen($part[2])<=1?'0'.$part[2]:$part[2];
        $time[0]=implode('-',$part);
        $date=$time[0];
        if (isset($time[1])){
            $date.=' '.$time[1];
...
function gregorianDate($time){
    $date=$time;
    if (is_rtl() && get_locale_key($GLOBALS['language'])=='fa' && isset($time) && $time!=null && $time!=''){
        $time=explode(' ',$time);
        $part=explode('-',$time[0]);
        $part=$this->jalali_to_gregorian($part[0],$part[1],$part[2]);
        $part[1]=strlen($part[1])<=1?'0'.$part[1]:$part[1];
        $part[2]=strlen($part[2])<=1?'0'.$part[2]:$part[2];
        $time[0]=implode('-',$part);
        $date=$time[0];
...
function gregorianToSolarMonths($month){
    if (is_rtl() && get_locale_key($GLOBALS['language'])=='fa' && isset($month) && $month!=null && $month!='') {
        $gregorianToSolarMonths = array(
            'January' => 'فروردین',
            'February' => 'اردیبهشت',
            'March' => 'خرداد',
            'April' => 'تیر',
        ...
        switch ($month) {
            case 'January':
                $solarMonth = $gregorianToSolarMonths['January'];
                break;
            case 'February':
                $solarMonth = $gregorianToSolarMonths['February'];
                break;
            case 'March':
                $solarMonth = $gregorianToSolarMonths['March'];
                break;
            case 'April':
                $solarMonth = $gregorianToSolarMonths['April'];
                break;
            case 'May':
                $solarMonth = $gregorianToSolarMonths['May'];
                break;
            case 'June':
                $solarMonth = $gregorianToSolarMonths['June'];
                break;
            case 'July':
                $solarMonth = $gregorianToSolarMonths['July'];
                break;
            case 'August':
                $solarMonth = $gregorianToSolarMonths['August'];
                break;
            case 'September':
                $solarMonth = $gregorianToSolarMonths['September'];
                break;
            case 'October':
                $solarMonth = $gregorianToSolarMonths['October'];
                break;
            case 'November':
                $solarMonth = $gregorianToSolarMonths['November'];
                break;
            case 'December':
                $solarMonth = $gregorianToSolarMonths['December'];
                break;
            default:
                $solarMonth = 'Invalid';
                break;
...
function jalaliYear($year){
    if (is_rtl() && get_locale_key($GLOBALS['language'])=='fa' && isset($year) && $year!=null && $year!=''){
        $part=$this->gregorian_to_jalali($year,'4','1');
        $year=$part[0];
    }
    return $year;
}
function isJalaliLeapYear($year='') {
    $year=$year??$this->jdate('Y');
    $remainder = $year % 33;
