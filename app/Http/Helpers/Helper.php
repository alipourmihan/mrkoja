...
if (!function_exists('createSlug')) {
  function createSlug($string)
  {
    ...
    $slug = preg_replace('/[^\p{L}\p{N}-]/u', '', $slug); // حذف کاراکترهای غیرمجاز
    $slug = preg_replace('/-+/', '-', $slug); // حذف خط تیره‌های تکراری
    $slug = trim($slug, '-'); // حذف خط تیره از ابتدا و انتها
...
if (!function_exists('applyLanguageFilter')) {
  /**
   * Apply language filter to a query only if the underlying table contains the language_id column.
   *
   ...
   * @return mixed
   */
  function applyLanguageFilter($query, string $table, $languageId)
  {
    if (!is_null($languageId) && Schema::hasColumn($table, 'language_id')) {
      $query->where($table . '.language_id', $languageId);
    }
...
if (!function_exists('vendorTotalAddedListing')) {
  ...
  {
    $total = Listing::where('user_id', $vendor_id)->get()->count();
    return $total;
  }
}
...
if (!function_exists('vendorTotalListing')) {
  ...
  {
    $vendorTotalListing = Listing::where('user_id', $vendorId)->count();
...
if (!function_exists('packageTotalFaqs')) {
  ...
  {
    $vendor_id = Listing::where('id', $listing_id)->pluck('user_id')->first();
    if ($vendor_id != 0) {
      $current_package = VendorPermissionHelper::packagePermission($vendor_id);
...
if (!function_exists('packageTotalListing')) {
  ...
  {
    if ($vendor_id == 0) {
      // برای admin محدودیتی نداریم
      return 99999999;
    }
    
    $current_package = VendorPermissionHelper::packagePermission($vendor_id);
    
    // بررسی اینکه آیا package یک object است یا collection خالی
    if (is_object($current_package) && !($current_package instanceof \Illuminate\Support\Collection)) {
      return $current_package->number_of_listing ?? 99999999;
    }
    
    // اگر collection خالی است یا null است
    return 99999999;
  }
}
...
if (!function_exists('packageTotalProductImage')) {
  function packageTotalProductImage($listing_id)
  {
    ...
    if ($vendor_id != 0) {
      ...
      // بررسی اینکه آیا package یک object است یا collection خالی
      if (is_object($current_package) && !($current_package instanceof \Illuminate\Support\Collection)) {
        $productImageLimit = $current_package->number_of_images_per_products ?? 99999999;
      } else {
...
if (!function_exists('packageTotalListingImage')) {
  function packageTotalListingImage($vendor_id)
  {
    ...
    // بررسی اینکه آیا package یک object است یا collection خالی
    if (is_object($current_package) && !($current_package instanceof \Illuminate\Support\Collection)) {
      return $current_package->number_of_images_per_listing ?? 99999999;
    }
