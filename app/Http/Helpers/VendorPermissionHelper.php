...
class VendorPermissionHelper
{
  ...
  public static function packagesDowngraded($vendorId)
  {
    ...
    if ($userCurrentPackage) {
      ...
      $listingImage = Listing::with(['galleries'])->where('user_id', $vendorId)->get();
      if ($listingImage) {
      ...
      if ($features && in_array('Products', $features)) {
        ...
        $listingProductImages = Listing::with(['listingProducts'])->where('user_id', $vendorId)->get();
      ...
      if ($features && in_array('Products', $features)) {
        $listingProducts = Listing::with(['listingProducts'])->where('user_id', $vendorId)->get();
        if ($listingProducts) {
      ...
      if ($features && in_array('FAQ', $features)) {
        ...
        foreach ($languages as $language) {
          ...
          $listingFaqs = Listing::with(['listingFaqs' => function ($q) use ($language_id) {
            ...
            ->where('listings.user_id', $vendorId)
      ...
      if ($features && in_array('Feature', $features)) {
        $ListingFeatures = Listing::with('specifications')->where('user_id', $vendorId)->select('id')->get();
        if ($ListingFeatures) {
      ...
      if ($features && in_array('Social Links', $features)) {
        $ListingSocialLinks = Listing::with('sociallinks')->where('user_id', $vendorId)->select('id')->get();
      ...
      if ($features && in_array('Amenities', $features)) {
        $ListingAmenities = Listing::with('listing_content')->where('user_id', $vendorId)->select('id')->get();
        if ($ListingAmenities) {
