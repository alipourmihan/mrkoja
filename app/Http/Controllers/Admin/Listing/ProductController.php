...
class ProductController extends Controller
{
    public function index(Request $request, $id)
    {
        ...
        if ($permissions) {
            ...
            Session::flash('warning', "دسترسی محصولات این فروشنده اعطا نشده است.");
    ...
    public function store(ProductStoreRequest $request)
    {
        if ($request->user_id != 0) {
            ...
            if ($totalproduct < packageTotalProducts($request->listing_id)) {
                ...
                Session::flash('warning', "حد محصولات به پایان رسیده یا از آن فراتر رفته است");
                return Response::json(['status' => 'success'], 200);
            }
        } else {
            DB::transaction(function () use ($request) {
    ...
    public function edit($id)
    {
        ...
        if ($permissions) {
            ...
            $information['vendors'] = \App\Models\User::businesses()->get();
            $information['currencyInfo'] = $this->getCurrencyInfo();
    ...
    public function bulkDelete(Request $request)
    {
        ...
        Session::flash('success', 'محصولات با موفقیت حذف شدند!');
