...
class VendorManagementController extends Controller
{
    ...
    public function update_setting(Request $request)
    {
        ...
        DB::table('basic_settings')->updateOrInsert(
            ...
            Session::flash('success', 'تنظیمات با موفقیت به‌روزرسانی شد!');
        return back();
    }
    ...
    public function create(Request $request)
    {
        ...
        Session::flash('success', 'فروشنده با موفقیت اضافه شد!');
    ...
    public function show($id)
    {
        ...
        $information['language'] = $language;
        $vendor = User::businesses()->with([
            'vendorInfo' => function ($query) use ($language) {
    ...
    public function updateAccountStatus(Request $request, $id)
    {
        ...
        $user = User::businesses()->find($id);
        if ($request->account_status == 1) {
            $user->update(['status' => 1]);
        } else {
            $user->update(['status' => 0]);
        }
        Session::flash('success', 'وضعیت حساب کاربری با موفقیت به‌روزرسانی شد!');
    ...
    {
        $vendor = User::businesses()->find($id);
        if ($request->email_status == 1) {
            $vendor->update(['email_verified_at' => now()]);
        } else {
            $vendor->update(['email_verified_at' => NULL]);
        }
        Session::flash('success', 'وضعیت ایمیل با موفقیت به‌روزرسانی شد!');

        return redirect()->back();
    }
    public function changePassword($id)
    {
        $userInfo = User::businesses()->findOrFail($id);
    ...
    {
        $information['languages'] = Language::get();
        $vendor = User::businesses()->where('id', $id)->firstOrFail();
        $information['vendor'] = $vendor;
        $information['currencyInfo'] = $this->getCurrencyInfo();
    ...
    public function update(Request $request, $id, Vendor $vendor)
    {
        ...
        $in = $request->all();
        $vendor  = User::businesses()->where('id', $id)->first();
        $file = $request->file('photo');
        if ($file) {
            $extension = $file->getClientOriginalExtension();
            $directory = public_path('assets/admin/img/vendor-photo/');
        ...
        Session::flash('success', 'فروشنده با موفقیت به‌روزرسانی شد!');
    ...
    public function addCurrPackage(Request $request)
    {
        ...
        $vendor = User::businesses()->where('id', $vendor_id)->first();
        $bs = Basic::first();
        ...
        Session::flash('success', 'پکیج فعلی با موفقیت اضافه شد!');
        return back();
    }
    ...
    {
        $vendor_id = $request->vendor_id;
        $vendor = User::businesses()->findOrFail($vendor_id);
        ...
        if (!empty($nextMembership) && $selectedPackage->term == 'lifetime') {
            Session::flash('warning', 'برای اضافه کردن پکیج مادام‌العمر به عنوان پکیج فعلی، باید پکیج بعدی را حذف کنید');
            return back();
        }
        ...
        Session::flash('success', 'پکیج فعلی با موفقیت تغییر کرد!');
        return back();
    }
    ...
    {
        $vendor_id = $request->vendor_id;
        $vendor = User::businesses()->where('id', $vendor_id)->firstOrFail();
        $currMembership = VendorPermissionHelper::currMembOrPending($vendor_id);
        ...
        Session::flash('success', 'پکیج فعلی با موفقیت حذف شد!');
        return back();
    }
    ...
    public function addNextPackage(Request $request)
    {
        ...
        if ($hasPendingMemb) {
            Session::flash('warning', 'این کاربر قبلا یک پکیج در حال پردازش دارد. لطفا ابتدا اقدام (تغییر / حذف / تایید / رد) به آن پکیج را انجام دهید.');
            return back();
        }
        ...
        if ($currMembership->is_trial == 1) {
            Session::flash('warning', 'اگر پکیج فعلی شما آزمایشی است، باید پکیج فعلی را ابتدا تغییر یا حذف کنید.');
            return back();
        }
        ...
        if ($currPackage->term != 'lifetime') {
            ...
            Session::flash('warning', 'اگر پکیج فعلی شما مادام‌العمر است، باید پکیج فعلی را ابتدا تغییر یا حذف کنید.');
            return back();
        }


        Session::flash('success', 'پکیج بعدی با موفقیت اضافه شد!');
        return back();
    }
    ...
    public function changeNextPackage(Request $request)
    {
        ...
        Session::flash('success', 'پکیج بعدی با موفقیت تغییر کرد!');
        return back();
    }
    ...
    public function removeNextPackage(Request $request)
    {
        ...
        Session::flash('success', 'پکیج بعدی با موفقیت حذف شد!');
        return back();
    }
    ...
    {
        $vendor = User::businesses()->findOrFail($id);
        // vendor memeberships
        ...
        return redirect()->back()->with('success', 'اطلاعات فروشنده با موفقیت حذف شد!');
    }
    ...
    public function bulkDestroy(Request $request)
    {
        ...
        Session::flash('success', 'اطلاعات فروشندگان با موفقیت حذف شد!');
