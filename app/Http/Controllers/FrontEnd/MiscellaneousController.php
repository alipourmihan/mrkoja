...
class MiscellaneousController extends Controller
{
  ...
  /**
   * Change language by direct code parameter
   *
   * @param Request $request
   * @param string $lang_code
   * @return \Illuminate\Http\RedirectResponse
   */
  public function changeLanguageByCode(Request $request, $lang_code)
  {
   
    // Only allow English and Persian
    if ($lang_code !== 'en' && $lang_code !== 'fa') {
      $lang_code = 'en'; // Default to English if invalid
    }
   
    // Set the language code in session
    $request->session()->put('currentLocaleCode', $lang_code);
    $request->session()->put('currentLangCode', $lang_code);
