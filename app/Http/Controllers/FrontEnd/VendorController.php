...
class VendorController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->filled('name')) {
            $name = $request->name;
            $u_infos = User::businesses()->where('users.username', 'like', '%' . $name . '%')->get();
            ...
            foreach ($v_infos as $v_info) {
                if (!in_array($v_info->user_id, $vendorIds)) {
                    array_push($vendorIds, $v_info->user_id);
                }
            }
        }
        ...
        if ($request->filled('location')) {
            ...
            foreach ($vendor_contents as $vendor_content) {
                if (!in_array($vendor_content->user_id, $vendorIds)) {
                    array_push($vendorIds, $vendor_content->user_id);
                }
            }
        }
        ...
        $information['vendors'] = User::businesses()
            ...
            ->when($name, function ($query) use ($vendorIds) {
                return $query->whereIn('users.id', $vendorIds);
            })
            ->when($location, function ($query) use ($vendorIds) {
                return $query->whereIn('users.id', $vendorIds);
            })
            ->where('users.id', '!=', 0)
            ->select('users.*', 'users.id as vendorId', 'memberships.*')
            ->paginate(10);
    ...
    public function details(Request $request)
    {
        ...
        if ($request->username == "admin") {
            ...
            $vendor = User::businesses()
                ...
                ->select('users.*')
                ->firstOrFail();
            $vendorInfo = VendorInfo::where([['user_id', $vendor->id], ['language_id', $language->id]])->first();
        ...
        $information['listings'] = Listing::where([['listings.user_id', $vendor_id], ['listings.status', '=', '1'], ['listings.visibility', '=', '1']])
            ->orderBy('id', 'desc')
            ->get();
