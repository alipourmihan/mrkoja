...
class ListingContoller extends Controller
{
    public function getState(Request $request)
    {
        ...
        if ($request->id) {
            ...
            $data['states'] = applyLanguageFilter(State::query(), 'states', $language->id)->get();
            $data['cities'] = applyLanguageFilter(City::query(), 'cities', $language->id)->get();
        }
    ...
    public function getCity(Request $request)
    {
        ...
        if ($request->id) {
            ...
            $data = applyLanguageFilter(City::query(), 'cities', $language->id)->get();
        }
        return $data;
    }
    ...
    public function index(Request $request)
    {
        ...
        $information['categories'] = ListingCategory::where(function ($query) use ($language) {
                if (Schema::hasColumn('listing_categories', 'language_id')) {
            ...
            ->orderBy('name')
            ->get();
        $information['vendors'] = \App\Models\User::businesses()->select('id as vendor_id', 'username')
            ->where('status', 1)
            ->orderBy('username')
            ->get();
        $information['states'] = applyLanguageFilter(State::query()->orderBy('name'), 'states', $language->id)->get();
        $information['cities'] = applyLanguageFilter(City::query()->orderBy('name'), 'cities', $language->id)->get();

        $title = $location = $category_id = $ratings = $amenitie = $vendor = $state = $city = null;
    ...
    public function search_listing(Request $request)
    {
        ...
        if ($request->filled('vendor')) {
            ...
            $listing_contents = Listing::where('user_id', $vendor)
                ->get()
                ->pluck('id');
            foreach ($listing_contents as $listing_content) {
    ...
    public function details($slug, $id)
    {
        ...
        $listing = Listing::with(['listing_content' => function ($query) use ($language) {
            ...
            ->when($vendorId && $vendorId != 0, function ($query) {
                $query->join('memberships', 'listings.user_id', '=', 'memberships.user_id')
                    ->where([
                        ['memberships.status', '=', 1],
                        ['memberships.start_date', '<=', now()->format('Y-m-d')],
                        ['memberships.expire_date', '>=', now()->format('Y-m-d')],
        ...
        if (is_null($listing_content)) {
            Session::flash('error', 'اطلاعاتی برای این لیست در زبان ' . $language->name . ' یافت نشد');
        ...
        if ($vendorId == 0) {
            ...
            $information['vendor'] = \App\Models\User::Where('id', $vendor_id)->first();
            $information['userName'] = $information['vendor']->username;
            $information['vendorInfo'] = VendorInfo::Where('user_id', $vendor_id)
    ...
    public function contact(Request $request)
    {
        ...
        if ($listing->user_id != 0) {
            $vendor = \App\Models\User::where('id', $listing->user_id)->select('to_mail', 'username', 'email')->first();
    ...
    public function productContact(Request $request)
    {
        ...
        if ($message->user_id != 0) {
            $vendor = \App\Models\User::where('id', $message->user_id)->select('to_mail', 'username', 'email')->first();
    ...
    public function storeReview(Request $request, $id)
    {
        ...
        if ($validator->fails()) {
            return redirect()->back()
                ->with('error', 'فیلد امتیاز الزامی است.')
                ->withInput();
        }
        ...
        if ($user) {
            ...
            Session::flash('error', 'شما باید ابتدا وارد شوید!');
        }
        return redirect()->back();
    }
    public function store_visitor(Request $request)
    {
        ...
        if ($listing) {
            if (!$check) {
                ...
                $visitor->user_id = $listing->user_id;
