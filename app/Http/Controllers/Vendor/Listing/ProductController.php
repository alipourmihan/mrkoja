...
class ProductController extends Controller
{
            ...
            if ($current_package != '[]') {
                ...
                if ($permissions) {
                    ...
                    $information['listing_products'] = ListingProduct::with([
                        ...
                        ->where('listing_id', $id)
                        ->where('listing_products.user_id', Auth::user()->id)
                        ->orderBy('id', 'desc')
                        ->paginate(10);
                    ...
                    Session::flash('warning', "شما هیچ دسترسی به محصولات ندارید.");
                    return redirect()->route('vendor.listing_management.listing');
                }
            } else {

                Session::flash('warning', 'لطفا یک پکیج خریداری کنید تا بتوانید محصولات را مدیریت کنید!');
                return redirect()->route('vendor.listing_management.listing');
            }
        } else {
    ...
    public function create($id)
    {
        ...
        if ($vendor_id == Auth::user()->id) {
            ...
            if ($current_package != '[]') {
                ...
                Session::flash('warning', 'لطفا یک پکیج خریداری کنید تا بتوانید محصولات را ایجاد کنید!');
                return redirect()->route('vendor.listing_management.listing');
            }
        } else {
            ...
            'file' => [
                function ($attribute, $value, $fail) use ($img, $allowedExts) {
                    ...
                    if (!in_array($ext, $allowedExts)) {
                        return $fail("فقط تصاویر png, jpg, jpeg مجوز دارند");
                    }
                },
            ]
        ];
    ...
    public function updateStatus(Request $request)
    {
        ...
        if ($request->status == 1) {
            ...
            Session::flash('success', 'محصول فعال شد!');
        } else {
            $listing->update(['status' => 0]);

            Session::flash('success', 'محصول غیرفعال شد!');
        }
    ...
    public function edit($id)
    {
        ...
        if ($current_package != '[]') {
            ...
            if ($permissions) {
                ...
                $listing = ListingProduct::with('galleries')->where('user_id', '=', Auth::user()->id)->findOrFail($id);
                $information['product'] = $listing;
                $information['product_id'] = $id;

                $information['languages'] = Language::all();

                $information['vendors'] = User::businesses()->get();
                $information['currencyInfo'] = $this->getCurrencyInfo();
            ...
            Session::flash('warning', 'لطفا یک پکیج خریداری کنید تا بتوانید محصولات را ویرایش کنید!');
            return redirect()->route('vendor.listing_management.listing');
        }
    }
    ...
    public function bulkDelete(Request $request)
    {
        ...
        Session::flash('success', 'محصولات با موفقیت حذف شد!');
