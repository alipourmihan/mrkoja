...
use App\Http\Helpers\JalaliDate;
...
class AppServiceProvider extends ServiceProvider
{
  ...
  public function boot()
  {
    ...
    // Add JalaliDate as a global alias
    if (!class_exists('JalaliDate')) {
      class_alias(JalaliDate::class, 'JalaliDate');
  }
    ...
    if (!app()->runningInConsole()) {
      ...
      View::composer('vendors.*', function ($view) {
        ...
        $dowgraded = [
            'listingImgDown' => [],
            'listingImgListingContents' => [],
            'listingProductDown' => [],
            'listingProductListingContents' => [],
            'listingFaqDown' => [],
            'listingFaqListingContents' => [],
            'featureDown' => [],
            'listingFeaturesListingContents' => [],
            'socialLinkDown' => [],
        ...
        if (Auth::check() && Auth::user() instanceof \App\Models\User && method_exists(Auth::user(), 'isBusiness') && Auth::user()->isBusiness()) {
            $dowgraded = array_merge($dowgraded, VendorPermissionHelper::packagesDowngraded(Auth::user()->id));
        }
