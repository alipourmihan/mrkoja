      ...
      <div class="row gx-xl-5">
        ...
        <div class="col-lg-4 col-xl-3"><!-- Section gap -->
          ...
          <aside class="widget-area" data-aos="fade-up">
            ...
            @php
              $mobilePhone = $listing->mobile_phone ?: $listing->phone;
              $landlinePhone = $listing->landline_phone;
              $emailAddress = $listing->mail;
              $websiteHref = null;
              if (!empty($listing->website)) {
                  $websiteHref = \Illuminate\Support\Str::startsWith($listing->website, ['http://', 'https://'])
                      ? $listing->website
                      : 'https://' . ltrim($listing->website, '/');
              }
              $instagramHref = null;
              $instagramLabel = null;
              if (!empty($listing->instagram)) {
                  $cleanInstagram = ltrim($listing->instagram, '@');
                  $instagramLabel = '@' . $cleanInstagram;
                  $instagramHref = \Illuminate\Support\Str::startsWith($listing->instagram, ['http://', 'https://'])
                      ? $listing->instagram
                      : 'https://instagram.com/' . $cleanInstagram;
              }
              $telegramHref = null;
              $telegramLabel = null;
              if (!empty($listing->telegram)) {
                  $cleanTelegram = ltrim($listing->telegram, '@');
                  $telegramLabel = '@' . $cleanTelegram;
                  $telegramHref = \Illuminate\Support\Str::startsWith($listing->telegram, ['http://', 'https://'])
                      ? $listing->telegram
                      : 'https://t.me/' . $cleanTelegram;
              }
              $whatsappHref = null;
              if (!empty($listing->whatsapp)) {
                  $whatsDigits = preg_replace('/\D+/', '', $listing->whatsapp);
                  if (!empty($whatsDigits)) {
                      $whatsappHref = 'https://wa.me/' . $whatsDigits;
                  }
              }
            @endphp
            @if ($mobilePhone || $landlinePhone || $emailAddress || $websiteHref || $instagramHref || $telegramHref || $whatsappHref)
              <div class="widget widget-contact radius-md mb-30">
                <h5 class="title">
                    ...
                    <ul class="list-group contact-list">
                      @if ($mobilePhone)
                        <li class="icon-start">
                          ...
                          <a href="tel:{{ preg_replace('/\s+/', '', $mobilePhone) }}">{{ $mobilePhone }}</a>
                        </li>
                      @endif
                      @if ($landlinePhone)
                        <li class="icon-start">
                          <i class="fal fa-phone-office"></i>
                          <a href="tel:{{ preg_replace('/\s+/', '', $landlinePhone) }}">{{ $landlinePhone }}</a>
                        </li>
                      @endif
                      @if ($emailAddress)
                        <li class="icon-start">
                      ...
                      @if ($websiteHref)
                        <li class="icon-start">
                          ...
                          <a href="{{ $websiteHref }}" target="_blank" rel="noopener">
                            {{ parse_url($websiteHref, PHP_URL_HOST) ?? $listing->website }}
                          </a>
                        </li>
                      @endif
                      @if ($instagramHref)
            ...
            @if (!empty($businessHours) && count($businessHours))
