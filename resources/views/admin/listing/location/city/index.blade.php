                    ...
                    <tbody>
                      @foreach ($cities as $city)
                        <tr>
                          ...
                          <td>
                            @php
                              $stateQuery = App\Models\Location\State::query()->where('country_id', $city->country_id);
                              if (!empty($stateHasLanguageColumn) && $stateHasLanguageColumn && isset($city->language_id)) {
                                  $stateQuery->where('language_id', $city->language_id);
                              }
                              $x = $stateQuery->count();
                              $okValue = $x != 0 ? 'OK' : null;
                            @endphp
                            @if($city->frontend_url)
                              <a class="btn btn-info btn-sm mr-1 mt-1" href="{{ $city->frontend_url }}" target="_blank" title="مشاهده صفحه">
