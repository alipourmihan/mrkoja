        ...
        <div class="card-pricing2 @if (isset($current_package->id) && $current_package->id === $package->id) card-success @else card-primary @endif">
          <div class="pricing-header">
            ...
            @if (isset($current_package->id) && $current_package->id === $package->id)
              <h3 class="badge badge-danger d-inline-block float-left ml-2">{{ __('Current') }}</h3>
            @endif
            @if ($package_count >= 2)
              @if ($next_package)
                @if ($next_package->id == $package->id)
                  <h3 class="badge badge-warning d-inline-block float-left ml-2">{{ __('Next') }}</h3>
                @endif
              @endif
            @endif
              ...
              @if (isset($current_package->id) && $current_package->id === $package->id)
                @if ($package->term != 'lifetime' || $current_membership->is_trial == 1)
                  <a href="javascript:void(0)"
                    class="btn btn-success btn-lg w-75 fw-bold mb-3 disabled">{{ __('Extend') }}</a>
                @endif
              @else
                <a href="javascript:void(0)"
                  class="btn btn-primary btn-block btn-lg fw-bold mb-3 disabled">{{ __('Buy Now') }}</a>
              @endif
            </div>
          @endif
        </div>
